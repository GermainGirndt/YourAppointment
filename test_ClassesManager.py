...
class test_add_student(unittest.TestCase):
	...
	right_inputs_student_id_0 = ["João", "da Silva", "23-05-1978", "297.586.890-10", "Bom Sucesso, 487", "Casa", "São Paulo", "SP"]	
	...
	def test_add_1_student_passes(self):
		with mock.patch('ClassesManager.input', side_effect=self.right_inputs_student_id_0):
	...
	def test_add_2_students_passes(self):
		...
		with mock.patch('ClassesManager.input', side_effect=self.right_inputs_student_id_1):
			self.erp_instance.add_student()
	...
	def test_add_1_student_fails_numbers_in_string(self):
		wrong_input = "João2"
		inputed_inputs = [wrong_input, "João", "da Silva", "23-05-1978", "297.586.890-10", "Bom Sucesso, 487", "Casa", "São Paulo", "SP"]
		awaited_exception = "Invalid input. The name muss contain only letter or spaces."
		self.held, sys.stdout = sys.stdout, StringIO()
		with mock.patch('ClassesManager.input', side_effect=inputed_inputs):
			self.erp_instance.add_student()
		self.assertEqual(self.erp_instance.students[-1], self.right_inputs_student_id_0)
		printed_messages = sys.stdout.getvalue()
		self.assertIn(awaited_exception, printed_messages)
...
class test_remove_student_by_id(unittest.TestCase):
	...
	def setUp(self):
		self.erp_instance = Erp()
		self.right_inputs_student_id_0 = ["João", "da Silva", "23-05-1978", "297.586.890-10", "Bom Sucesso, 487", "Casa", "São Paulo", "SP"]
	...
	def test_remove_1_student_by_id_fails_inexisting_Id(self):
		...
		awaited_exception = f"The inputed value is out of range. Please input a index from 0 to {len(self.erp_instance.students)}"
		self.held, sys.stdout = sys.stdout, StringIO()
		with mock.patch('ClassesManager.input', side_effect=inputed_inputs):
			self.erp_instance.remove_student_by_id()
		printed_messages = sys.stdout.getvalue()
	...
	def test_remove_1_student_by_id_fails_valueerror(self):
		wrong_input = ["lala"]
		with mock.patch('ClassesManager.input', side_effect=wrong_input):
...
class test_remove_student_by_name(unittest.TestCase):
	...
	def setUp(self):
		...
		right_inputs_student_id_0 = ["João", "da Silva", "23-05-1978", "297.586.890-10", "Bom Sucesso, 487", "Casa", "São Paulo", "SP"]
		right_inputs_student_id_1 = ["Joana", "Silveira", "17-02-1972", "434.763.780-20", "Felicidade, 14", "Ap. 201", "Rio de Janeiro", "RJ"]
		with mock.patch('ClassesManager.input', side_effect=right_inputs_student_id_0):
			self.erp_instance.add_student()
		with mock.patch('ClassesManager.input', side_effect=right_inputs_student_id_1):
			self.erp_instance.add_student()
